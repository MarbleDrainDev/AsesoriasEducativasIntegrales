(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=`modulepreload`,t=function(e){return`/AsesoriasEducativasIntegrales/`+e},n={};const r=function(r,i,a){let o=Promise.resolve();if(i&&i.length>0){let r=document.getElementsByTagName(`link`),s=document.querySelector(`meta[property=csp-nonce]`),c=s?.nonce||s?.getAttribute(`nonce`);function l(e){return Promise.all(e.map(e=>Promise.resolve(e).then(e=>({status:`fulfilled`,value:e}),e=>({status:`rejected`,reason:e}))))}o=l(i.map(i=>{if(i=t(i,a),i in n)return;n[i]=!0;let o=i.endsWith(`.css`),s=o?`[rel="stylesheet"]`:``;if(a)for(let e=r.length-1;e>=0;e--){let t=r[e];if(t.href===i&&(!o||t.rel===`stylesheet`))return}else if(document.querySelector(`link[href="${i}"]${s}`))return;let l=document.createElement(`link`);if(l.rel=o?`stylesheet`:e,o||(l.as=`script`),l.crossOrigin=``,l.href=i,c&&l.setAttribute(`nonce`,c),document.head.appendChild(l),o)return new Promise((e,t)=>{l.addEventListener(`load`,e),l.addEventListener(`error`,()=>t(Error(`Unable to preload CSS for ${i}`)))})}))}function s(e){let t=new Event(`vite:preloadError`,{cancelable:!0});if(t.payload=e,window.dispatchEvent(t),!t.defaultPrevented)throw e}return o.then(e=>{for(let t of e||[])t.status===`rejected`&&s(t.reason);return r().catch(s)})};var i=document.querySelector(`#app`),a=`/AsesoriasEducativasIntegrales/`,o=e=>{if(!e)return e;let t=e.match(/youtube\.com\/watch\?v=([a-zA-Z0-9_-]+)/);if(t){let n=t[1],r=e.match(/[&?]list=([a-zA-Z0-9_-]+)/);return`https://www.youtube.com/embed/${n}${r?`?list=${r[1]}`:``}`}let n=e.match(/youtu\.be\/([a-zA-Z0-9_-]+)/);if(n){let t=n[1],r=e.match(/[&?]list=([a-zA-Z0-9_-]+)/);return`https://www.youtube.com/embed/${t}${r?`?list=${r[1]}`:``}`}return e},s=e=>e&&(/^(https?:|mailto:|tel:|data:|#)/i.test(e)?o(e):e.startsWith(`/`)?`${a}${e.slice(1)}`:`${a}${e}`),c=e=>{if(!e)return``;let t=e.split(`
`),n=``,r=!1,i=!1,a=e=>e.replace(/:warning:/g,`⚠️`).replace(/:memo:/g,`📝`).replace(/:bulb:/g,`💡`);for(let e of t){let t=e.trim();if(!t){r&&=(n+=`</ul>`,!1),i&&=(n+=`</blockquote>`,!1);continue}if(t.startsWith(`<`)&&t.endsWith(`>`)){r&&=(n+=`</ul>`,!1),i&&=(n+=`</blockquote>`,!1);let e=t.replace(/\b(src|href)=("|')([^"']+)(\2)/g,(e,t,n,r)=>`${t}=${n}${s(r)}${n}`);n+=a(e);continue}if(t.startsWith(`>`)){i||=(n+=`<blockquote>`,!0);let e=a(t.replace(/^>\s?/,``)).replace(/!\[(.*?)\]\((.*?)\)/g,(e,t,n)=>`<img src="${s(n)}" alt="${t}" loading="lazy" />`).replace(/\[(.*?)\]\((.*?)\)/g,(e,t,n)=>`<a href="${s(n)}">${t}</a>`).replace(/\*\*(.*?)\*\*/g,`<strong>$1</strong>`).replace(/\*(.*?)\*/g,`<em>$1</em>`);n+=`<p>${e}</p>`;continue}if(t.startsWith(`### `)){n+=`<h3>${t.slice(4)}</h3>`;continue}if(t.startsWith(`## `)){n+=`<h2>${t.slice(3)}</h2>`;continue}if(t.startsWith(`# `)){n+=`<h1>${t.slice(2)}</h1>`;continue}if(t.startsWith(`- `)){r||(r=!0,n+=`<ul>`),n+=`<li>${t.slice(2)}</li>`;continue}let o=a(t).replace(/!\[(.*?)\]\((.*?)\)/g,(e,t,n)=>`<img src="${s(n)}" alt="${t}" loading="lazy" />`).replace(/\[(.*?)\]\((.*?)\)/g,(e,t,n)=>`<a href="${s(n)}">${t}</a>`).replace(/\*\*(.*?)\*\*/g,`<strong>$1</strong>`).replace(/\*(.*?)\*/g,`<em>$1</em>`);n+=`<p>${o}</p>`}return r&&(n+=`</ul>`),n},l=Object.assign({"./sections/ConceptosMD.md":()=>r(()=>import(`./ConceptosMD-CAjuX91L.js`),[]).then(e=>e.default),"./sections/Contabilidad.md":()=>r(()=>import(`./Contabilidad-CVKISGyl.js`),[]).then(e=>e.default),"./sections/Emprendimiento.md":()=>r(()=>import(`./Emprendimiento-D0Ht1c-i.js`),[]).then(e=>e.default),"./sections/Prueba.md":()=>r(()=>import(`./Prueba-B2kmwUVX.js`),[]).then(e=>e.default),"./sections/Test.md":()=>r(()=>import(`./Test-CrbSapOt.js`),[]).then(e=>e.default),"./sections/servicios-extra.md":()=>r(()=>import(`./servicios-extra-B0UiajKW.js`),[]).then(e=>e.default)}),u=()=>Object.keys(l).map(e=>e.split(`/`).pop()).map(e=>e.replace(/\.md$/i,``)).filter(Boolean),d=async e=>{let t=l[`./sections/${e}.md`];if(!t)return null;let n=await t();return n?.trim()?n:null},f=async e=>{let t=e.querySelectorAll(`[data-md-src]`);for(let e of t){let t=e.getAttribute(`data-md-src`);if(!t)continue;let n=await d(t);n&&(e.innerHTML=c(n))}},p=e=>{let t=e.replace(/[-_]+/g,` `).trim();return t.charAt(0).toUpperCase()+t.slice(1)},m=e=>{let t=document.createElement(`section`);return t.className=`section`,t.id=e,t.dataset.view=e,t.innerHTML=`
...
width="100%" height="500" type="application/pdf"/&gt;</code></pre>
  ...
  `;let e=i.querySelector(`#site-menu`),t=i.querySelector(`.menu-toggle`),n=i.querySelector(`#main-content`),r=i.querySelector(`.side-list`),o=(e,t)=>{if(r.querySelector(`[data-target="${e}"]`))return;let n=document.createElement(`button`);n.className=`side-link`,n.type=`button`,n.dataset.target=e,n.textContent=t,r.appendChild(n)},s=e=>{r.querySelectorAll(`.side-link`).forEach(t=>{t.classList.toggle(`is-active`,t.dataset.target===e)})},c=t=>{let r=[...n.querySelectorAll(`.section`)],i=!1;r.forEach(e=>{let n=e.id===t;n&&(i=!0),e.classList.toggle(`is-active`,n)}),!i&&r.length>0&&(r[0].classList.add(`is-active`),t=r[0].id),e.querySelectorAll(`a`).forEach(e=>{e.classList.toggle(`is-active`,e.dataset.view===t)}),s(t);try{localStorage.setItem(`lastView`,t)}catch{}},l=(()=>{try{return localStorage.getItem(`lastView`)||``}catch{return``}})(),d=window.location.hash?.replace(`#`,``)||l||`inicio`;d&&c(d),e.addEventListener(`click`,n=>{if(n.target.matches(`a`)){n.preventDefault();let r=n.target.dataset.view;r&&(window.location.hash=r,c(r),e.classList.remove(`open`),t.setAttribute(`aria-expanded`,`false`))}}),t.addEventListener(`click`,()=>{let n=e.classList.toggle(`open`);t.setAttribute(`aria-expanded`,String(n))}),r.addEventListener(`click`,e=>{if(e.target.matches(`.side-link`)){let t=e.target.dataset.target;t&&(window.location.hash=t,c(t))}}),i.addEventListener(`click`,e=>{if(e.target.matches(`.copy-btn`)){let t=e.target.getAttribute(`data-code`);if(t){let n=t.replace(/&#10;/g,`
`);navigator.clipboard.writeText(n).then(()=>{let t=e.target.textContent;e.target.textContent=`✓ Copiado`,setTimeout(()=>{e.target.textContent=t},2e3)})}}}),window.addEventListener(`hashchange`,()=>{let e=window.location.hash?.replace(`#`,``);e&&c(e)});let h=new Set([...i.querySelectorAll(`[data-md-src]`)].map(e=>e.getAttribute(`data-md-src`)).filter(Boolean)),g=u().filter(e=>!h.has(e));if(g.length)for(let e of g){let t=m(e);n.appendChild(t),o(e,p(e))}await f(i)})();